...
- [ ] *(Recommended)* Harden the `center-x-equine-therapy-site` repo: its root mixes the deployed `public/` with sensitive internal docs (HIPAA vendor CSV, client PII PDF). Deploy is safe today (serves `public/`), but consider moving root planning docs into a clearly-private subfolder and pinning the Pages build output dir to `public/` so a future misconfig can't leak them.
